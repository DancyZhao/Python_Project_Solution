...
# 1. How many records are there in the dataset?
    #YOUR CODE HERE
data.fillna(data.mean(numeric_only=True), inplace=True) # Missing values

for col in data.select_dtypes(include=['object']).columns:
    data[col].fillna(data[col].mode()[0], inplace=True) # Missing values
...
try:
    ...
    raise ZeroDivisionError() # Exception handling
...
if 'HealthRating' in data.columns:
    unique_ratings = sorted(data['HealthRating'].unique())
    num_bins = len(unique_ratings)
...
for status in membership_statuses:
    ...
    if len(group_male) < 30 and len(group_female) < 30:
        z_stat, p_value = ztest(group_male, group_female) # Significance test
